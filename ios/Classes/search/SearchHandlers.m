...
@implementation UpdatePrivacyStatement
...
- (void)onMethodCall:(FlutterMethodCall *)call :(FlutterResult)result {
    ...
    NSArray<AMapGeoPolygon *> *avoidPolygons = [NSArray array];
    for (NSArray<AMapGeoPoint *> *array in _routePlanParam.avoidPolygons) {
        [avoidPolygons arrayByAddingObject: [AMapGeoPolygon polygonWithPoints:array]];
    }
    routeQuery.avoidpolygons = avoidPolygons;
    
    routeQuery.avoidroad = _routePlanParam.avoidRoad;
    routeQuery.showFieldType = AMapDrivingRouteShowFieldTypeAll;
...
- (void)onRouteSearchDone:(AMapRouteSearchBaseRequest *)request response:(AMapRouteSearchResponse *)response {
    if (response.route.paths.count == 0) {
        return _result([FlutterError errorWithCode:@"没有规划出合适的路线"
                                           message:@"没有规划出合适的路线"
                                           details:nil]);
    }
...
- (void)AMapSearchRequest:(id)request didFailWithError:(NSError *)error {
    if (_result != nil) {
        _result([FlutterError errorWithCode:[NSString stringWithFormat:@"%@", @(error.code)]
                                           message:[Misc toAMapErrorDesc:error.code]
                                           details:nil]);
    }
}
